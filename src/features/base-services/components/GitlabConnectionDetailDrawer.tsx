...
import type { GitlabConnection } from '@/services/api'
...
export function GitlabConnectionDetailDrawer({
  open,
  loading,
  error,
  connection,
  ...
  return (
    <Drawer title="GitLab 连接详情" open={open} onClose={onClose} size={560}>
      {loading ? <Skeleton active paragraph={{ rows: 9 }} /> : null}
      ...
      {!loading && !error && connection ? (
        <div className="base-services-detail-layout">
          ...
          <Descriptions column={1} bordered size="middle" className="base-services-detail-descriptions">
            ...
            <Descriptions.Item label="GitLab 地址">{connection.baseUrl || '-'}</Descriptions.Item>
            <Descriptions.Item label="鉴权方式">{connection.authType || '-'}</Descriptions.Item>
            <Descriptions.Item label="状态">
              <Tag color={statusMeta.color}>{statusMeta.label}</Tag>
            </Descriptions.Item>
            <Descriptions.Item label="Personal Access Token">
              <Tag color={connection.hasAccessToken ? 'success' : 'default'}>
                {connection.hasAccessToken ? '已配置' : '未配置'}
              </Tag>
            </Descriptions.Item>
            <Descriptions.Item label="最近验证时间">{formatTime(connection.lastAuthAt)}</Descriptions.Item>
