...
import { ApiEnvironmentDrawer } from '../components/ApiEnvironmentDrawer'
import { CollectionDrawer, type CollectionFormValues } from '../components/EntityDrawers'
...
export function ApiAutomationPage() {
  ...
  const latestSprint = useMemo(() => pickLatestItem(sprints, pickCreatedAt), [sprints])
  ...
  const latestRequirement = useMemo(() => pickLatestItem(requirements, pickCreatedAt), [requirements])
  ...
  const allRequirements = allRequirementsQuery.data ?? []
  ...
  const sprintFilterOptions = useMemo(
    () => [{ label: '全部迭代', value: 'all' }, ...sprints.map((sprint) => ({ label: sprint.name, value: normalizeSprintId(sprint) }))],
    [sprints],
  )
  ...
  const saveCollectionMutation = useMutation({
    ...
    onSuccess: () => {
      message.success(editingCollection ? 'Collection 已更新' : 'Collection 已创建')
      setDrawerOpen(false)
  ...
  const deleteCollectionMutation = useMutation({
    ...
    onSuccess: () => {
      message.success('Collection 已删除')
      queryClient.invalidateQueries({ queryKey: ['apiCollections'] })
    },
  })
  ...
  const runCollectionMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.status === 'running') {
        message.success('已开始运行，可在集合详情查看运行记录')
        return
      }
      message.success('集合运行已触发')
    },
    onSettled: () => {
  ...
  return (
    <div className="workbench-page api-automation-page">
        ...
        <section className="workbench-panel workbench-board-panel">
          <div className="panel-header api-panel-header">
            <div className="requirement-panel-head">
              <Text strong>ApiCollection 列表</Text>
                  ...
                  <Select
                    ...
                    onChange={(value) => {
                      ...
                      setRequirementSelection({
                        projectId: activeProjectId,
                        value: value === 'all' ? null : undefined,
                      })
                      setPage(1)
                    }}
                  />
          ...
          <div className="table-body-scroll sprint-card-scroll">
            {sprintsQuery.isLoading ? (
              ...
              <div className="sprint-card-loading">
                <Empty description="Collection 加载中..." image={Empty.PRESENTED_IMAGE_SIMPLE} />
              </div>
            ) : collections.length === 0 ? (
              <div className="sprint-card-loading">
                <Empty description="当前需求下暂无 Collection" />
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <Paragraph className="api-collection-description" type="secondary">
                        {collection.description || '暂无 Collection 描述'}
                      </Paragraph>
                      ...
                      >
                        <Tooltip title={resolvedEnvironmentId ? '运行集合' : '请先选择环境'}>
                          <Button
                            type="text"
                            shape="circle"
                        ...
                        <Popconfirm title="确认删除该 Collection？" onConfirm={() => deleteCollectionMutation.mutate(collectionId)}>
                          <Tooltip title="删除">
                            <Button
                              danger
                              type="text"
      ...
      <CollectionDrawer
        title={editingCollection ? '编辑 Collection' : '新建 Collection'}
        open={drawerOpen}
        form={drawerForm}
