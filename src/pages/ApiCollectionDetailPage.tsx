import { ArrowLeftOutlined, CodeOutlined, DeleteOutlined, DownloadOutlined, DownOutlined, EditOutlined, PlusOutlined, SearchOutlined, SendOutlined } from '@ant-design/icons'
import { Alert, Button, Card, Dropdown, Empty, Form, Input, InputNumber, Modal, Popconfirm, Popover, Segmented, Select, Switch, Tabs, Tag, Tooltip, Typography, message } from 'antd'
...
const DRAFT_CASE_ID = '__draft_case__'
const EMPTY_API_CASES: ApiCase[] = []
const EMPTY_API_ENVIRONMENTS: ApiEnvironment[] = []
const EMPTY_ASSERT_RULES: ApiAssertRule[] = []
const EMPTY_EXTRACT_RULES: ApiExtractRule[] = []
const MIN_EDITOR_TOP_HEIGHT = 280
const MIN_EDITOR_RESULT_HEIGHT = 48
const ENV_VAR_TOKEN_PREFIX = '{{'
const ENV_VAR_TOKEN_SUFFIX = '}}'
...
type RunResultView = 'request' | 'response' | 'extract' | 'assert'
type CollectionReportView = 'items' | 'runtime'
type EnvVarPickerMode = 'environment' | 'builtin'

const builtinTemplateFunctions: Array<{
  token: string
  label: string
  description: string
  example?: string
...
function createDefaultCaseFormValues(): ApiCaseFormValues {
  return {
    name: '',
    method: 'POST',
    path: '',
...
function stringifyKeyValuePairs(items?: Array<{ enabled?: boolean; key?: string; value?: string }>) {
...
function serializeCaseValues(values: ApiCaseFormValues): CreateApiCasePayload {
  return {
    name: values.name,
...
function buildApiCaseUpdatePayload(apiCase: ApiCase, values: ApiCaseFormValues): UpdateApiCasePayload {
  ...
  if (currentValues.priority !== values.priority) payload.priority = nextValues.priority
  ...
  if (normalizeText(currentValues.tagsJson) !== normalizeText(values.tagsJson)) payload.tagsJson = nextValues.tagsJson ?? ''
  ...
  if (currentValues.continueOnFailure !== values.continueOnFailure) {
    payload.continueOnFailure = nextValues.continueOnFailure
  }

  return payload
}

function mergeApiCaseWithFormValues(apiCase: ApiCase | null | undefined, values: ApiCaseFormValues): ApiCase {
...
function mergeDefinedApiCaseFields(baseCase: ApiCase, nextCase: ApiCase): ApiCase {
...
const bodyTypeOptions: Array<{ label: string; value: NonNullable<ApiCase['bodyType']> }> = [
...
const runResultViewOptions: Array<{ label: string; value: RunResultView }> = [
...
const collectionReportViewOptions: Array<{ label: string; value: CollectionReportView }> = [
  { label: '用例明细', value: 'items' },
  { label: '运行变量快照', value: 'runtime' },
]

const assertSourceOptions: Array<{ label: string; value: ApiAssertSource }> = [
...
const assertComparatorOptions: Array<{ label: string; value: ApiAssertComparator }> = [
  { label: '等于', value: 'eq' },
  { label: '不等于', value: 'neq' },
...
const extractSourceOptions: Array<{ label: string; value: ApiExtractRuleSource }> = [
  { label: '响应头', value: 'header' },
  { label: 'JSONPath', value: 'body_jsonpath' },
  { label: '响应文本', value: 'body_text' },
  { label: '状态码', value: 'status_code' },
...
const assertSourceLabelMap: Record<ApiAssertSource, string> = {
  status_code: '状态码',
  header: '响应头',
...
const assertComparatorLabelMap: Record<ApiAssertComparator, string> = {
  eq: '等于',
  neq: '不等于',
  contains: '包含',
  not_contains: '不包含',
...
const extractSourceLabelMap: Record<ApiExtractRuleSource, string> = {
  header: '响应头',
  body_jsonpath: 'JSONPath',
  body_text: '响应文本',
  status_code: '状态码',
...
function methodTagColor(method: ApiCase['method']) {
  switch (method) {
    case 'GET':
      return 'green'
    case 'POST':
...
function getCaseId(apiCase: ApiCase) {
  return apiCase.caseId ?? apiCase.case_id ?? ''
}

function getCollectionRunItemKey(item: ApiCollectionRunItem, index: number) {
  return item.itemId || item.caseRunId || `${item.caseId ?? 'case'}-${item.orderNo ?? index}-${index}`
}

function getCaseDisplayPath(urlTemplate: string) {
...
function buildCaseFormValues(apiCase: ApiCase): ApiCaseFormValues {
  ...
  return {
    ...
    priority: apiCase.priority ?? defaults.priority,
    ...
    tagsJson: apiCase.tagsJson ?? defaults.tagsJson,
...
function sortCasesByOrderNo(cases: ApiCase[]) {
  return [...cases].sort(
    (left, right) =>
...
function parseMaybeJsonValue(value?: string) {
  if (!value) return ''
...
function sortRulesByOrderNo<T extends { orderNo?: number; createdAt?: string; created_at?: string }>(rules: T[]) {
  return [...rules].sort((left, right) => {
    ...
    const leftCreated = pickCreatedAt(left)
    const rightCreated = pickCreatedAt(right)
    const leftTime = leftCreated ? new Date(leftCreated).getTime() : 0
    const rightTime = rightCreated ? new Date(rightCreated).getTime() : 0
...
function sanitizeFileName(value: string) {
  return value.replace(/[<>:"/\\|?*\u0000-\u001F]/g, '-').trim() || 'collection-report'
}

function buildCollectionRunReportHtml(params: {
...
<head>
  ...
  <title>${escapeHtml(collectionName)} - 集合报告</title>
  <style>
    :root {
...
<body>
  <div class="page">
    <section class="hero">
      <div class="hero-top">
        <h1>${escapeHtml(collectionName)} · 集合报告</h1>
...
export function ApiCollectionDetailPage() {
  ...
  const environmentVars = environmentVarsQuery.data ?? []
  ...
  }, [collectionRunReportQuery.data?.collectionRunId])
  ...
  function handleExportCollectionRunReportHtml() {
    ...
    const environmentName =
      ...
      '-'
    const collectionName = collectionQuery.data?.name || 'API 集合'
...
{JSON.stringify(parseMaybeJsonValue(response?.body), null, 2)}
  ...
  const filteredCases = useMemo(() => {
    ...
    return cases.filter((item) =>
      [item.name, item.urlTemplate, item.description, item.method, item.priority]
        .filter(Boolean)
    ...
    () =>
      draftCaseValues
        ? {
            ...
            priority: draftCaseValues.priority || 'P1',
    ...
    <div className="workbench-page api-collection-detail-page">
                    ...
                    <Card className="detail-block api-case-summary-card">
                      <div className="api-case-summary-grid compact">
                        ...
                        <div className="api-summary-item">
                          ...
                          <strong>{collectionQuery.data.version || '-'}</strong>
                        </div>
                        <div className="api-summary-item">
      ...
      >
        {collectionRunReportQuery.error && !collectionRunReport ? (
          ...
          <Empty description="集合报告加载中..." image={Empty.PRESENTED_IMAGE_SIMPLE} />
        ) : collectionRunReport ? (
...
{JSON.stringify(parseMaybeJsonValue(collectionRunReport.runtimeVarsJson), null, 2)}
                  ...
                  <Empty description="暂无集合运行明细" image={Empty.PRESENTED_IMAGE_SIMPLE} />
                ) : (
          ...
          <Empty description={selectedCollectionRunSummary?.status === 'running' ? '运行中，报告生成中...' : '暂无集合报告'} image={Empty.PRESENTED_IMAGE_SIMPLE} />
        )}
      </Modal>
      ...
      >
        ...
        >
          ...
          <Form.Item name="targetScope" hidden>
            <Input />
          </Form.Item>
        </Form>
      </Modal>
